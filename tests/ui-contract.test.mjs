...
test("title area exposes a silent corner stage switch", () => {
  ...
  assert.match(appJs, /const pageSwitchSlotIndex = LEVEL_PAGES\.length === 2 && index === 1 \? PAGE_SWITCH_SLOT_COUNT - 1 : index/);
  assert.match(appJs, /button\.style\.setProperty\("--page-switch-left", `\$\{\(pageSwitchSlotIndex \* 100\) \/ PAGE_SWITCH_SLOT_COUNT\}%`\)/);
  ...
  assert.match(appJs, /class: "page-switch-base-halo"/);
  assert.match(appJs, /class: "page-switch-lever"/);
  assert.match(appJs, /class: "page-switch-ball"/);
  ...
  assert.match(css, /transform:\s*translateX\(36px\) rotate\(48deg\)/);
...
test("level 41 advanced rhythms render as connected notation glyphs", () => {
  ...
  assert.match(appJs, /eighthRestOutline/);
  assert.match(appJs, /className\("eighth-rest-outline"\)/);
